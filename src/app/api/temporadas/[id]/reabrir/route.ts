...
export async function POST(
  ...
  try {
    const temporada = await reabrirTemporada(id, organizadorOuResposta.id);
    if (!temporada) {
    ...
    if (
      error instanceof TemporadaJaAbertaError ||
      error instanceof JaExisteTemporadaAbertaError
    ) {
